...
class ServersView(FlaskView):
    ...
    @conditional(auth.login_required, auth_enabled)
    @route('<int:id>/user/<string:pseudo>/mute', methods=['POST'])
    def user_mute_user(self, id, pseudo):
        """ Mutes a user
        """
        ...
        user = None
        for u in list(server.getUsers().values()):
            if u.name == pseudo:
                user = u
    ...
    @conditional(auth.login_required, auth_enabled)
    @route('<int:id>/user/<string:pseudo>/unmute', methods=['POST'])
    def user_unmute_user(self, id, pseudo):
        """ Unmutes a user
        """
